...
export function getActivationQuote(model: ComputeModelOption) {
  if (model.tier === "premium" || model.teeRequired) return { deposit: "3", protocolFee: "0.1", label: "Premium / TEE activation" };
  if (model.modality === "vision" || model.modality === "image" || model.modality === "audio") return { deposit: "2", protocolFee: "0.05", label: "Multimodal activation" };
  return { deposit: "1", protocolFee: "0.05", label: "Standard activation" };
}
